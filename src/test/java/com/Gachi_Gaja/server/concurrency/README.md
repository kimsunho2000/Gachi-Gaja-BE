...
그래서 이 테스트들만 `concurrency` 프로파일(`application-concurrency.properties`, MySQL)을 쓴다.
`ConcurrencyTestSupport` 의 `@ActiveProfiles("concurrency")` 가 자동으로 걸어주므로 별도 지정은 필요 없다.
나머지 단위/슬라이스 테스트는 기본 `test` 프로파일(`application-test.properties`, H2 인메모리)에서 돈다.
...
> **3초는 어디서 오는가.** `GroupRepository` 의 `@QueryHint(jakarta.persistence.lock.timeout)` 이 아니다.
> Hibernate 의 MySQL 다이얼렉트는 그 힌트에서 `0`(NOWAIT)과 `-2`(SKIP LOCKED)만 SQL 로 옮기고
> `3000` 같은 값은 대응하는 MySQL 문법이 없어 버린다 — 실제로 나가는 SQL 은 `for update` 평문이다.
> 그대로 두면 서버 기본값 `innodb_lock_wait_timeout=50` 이 걸려 요청 하나가 최대 50초간 스레드를 점유한다.
> 그래서 커넥션 세션 변수로 지정한다.
>
> ```properties
...
> ```
>
> NOWAIT(`value = "0"`)로 바꾸면 대기 없이 즉시 실패하지만, 재시도를 붙이지 않으면
> 빈 그룹에 10명이 동시에 들어올 때 **1명만 가입되고 9명이 튕긴다**(정원은 지켜지되 자리가 남는다).
> 대기 한도를 주는 쪽을 택한 이유다.
